...
export default function PurchasingTab({
  ...
  // Confirm accounting for purchases that came from the scheduler (goods already received)
  // This creates only the voucher entry WITHOUT updating stock (stock was already updated at receiving time)
  const handleConfirmSchedulerPurchase = async (purchase: Purchase) => {
    if (!confirm(`確認對「${purchase.vendor}」的待入帳貨單建立傳票？\n⚠️ 庫存數量不會再次增加（貨物已於簽收時入庫）`)) return;
    try {
      const findAccountName = (id: string, fallback: string) => {
        const acct = coa.find((c: any) => c.id === id || c.code === id);
        return acct ? (acct.name || acct.label || fallback) : fallback;
      };

      // Build entry lines (same logic as normal purchase)
      const totalAmt = purchase.totalAmount || 0;
      const entryLines: any[] = [
        {
          id: uid(),
          type: 'debit',
          ...
          accountName: findAccountName('5101', '食材成本'),
          amount: totalAmt,
          lineDescription: `${purchase.vendor} 進貨 (排班簽收)`
        },
        purchase.paymentType === '現結'
          ? {
              id: uid(),
              type: 'credit',
              accountId: '1101',
              accountName: findAccountName('1101', '現金'),
              amount: totalAmt,
              lineDescription: `${purchase.vendor} 進貨現結`
            }
          : {
              id: uid(),
              type: 'credit',
              ...
              accountName: findAccountName('2101', '應付帳款'),
              amount: totalAmt,
              lineDescription: `${purchase.vendor} 進貨月結`
            }
      ];

      const dateParts = (purchase.date || new Date().toISOString().split('T')[0]).split('-');
      const yy = dateParts[0].slice(-2);
      const mm = dateParts[1];
      const dd = dateParts[2];
      const datePrefix = `${yy}${mm}${dd}`;
      const count = (await getDocs(collection(db, 'shops', shopId, 'entries'))).size;
      const voucherNo = `${datePrefix}-${String(count + 1).padStart(4, '0')}`;

      const entryPayload = {
        id: purchase.id,
        date: purchase.date,
        year: purchase.year || new Date().getFullYear(),
        voucherNo,
        description: `${purchase.vendor} 排班系統簽收進貨傳票`,
        lines: entryLines,
        debitTotal: totalAmt,
      ...
      const batch = writeBatch(db);
      batch.set(doc(db, 'shops', shopId, 'entries', purchase.id), entryPayload);
      // Mark accountingStatus as confirmed (do NOT update stock)
      batch.update(doc(db, 'shops', shopId, 'purchases', purchase.id), {
        accountingStatus: 'confirmed'
      });
      await batch.commit();
      alert(`✅ 傳票已建立！廠商：${purchase.vendor}，金額：$${totalAmt.toLocaleString()}。庫存不變（已於簽收時更新）。`);
    } catch (err) {
      console.error(err);
      alert('傳票建立失敗，請重試！');
    }
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="glass-panel p-4 md:p-6 bg-white border border-coffee-50 shadow-sm overflow-y-auto max-h-[600px] rounded-[24px]">
          ...
          {/* Pending Accounting from Scheduler - shown prominently at top */}
          {(() => {
            const pendingAcctPurchases = purchases.filter(p => p.fromScheduler && p.accountingStatus === 'pending');
            if (pendingAcctPurchases.length === 0) return null;
            return (
              <div className="mb-5 bg-amber-50 border border-amber-200 rounded-2xl p-4 flex flex-col gap-3">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <h4 className="font-extrabold text-amber-800 text-sm">📦 待入帳貨單</h4>
                      <p className="text-[10px] text-amber-600">來自排班系統的簽收貨物，庫存已更新，需要確認入帳並建立傳票。</p>
                    </div>
                  </div>
                  <span className="text-[10px] font-bold text-amber-700 bg-amber-200 px-2 py-0.5 rounded-full">{pendingAcctPurchases.length} 筆</span>
                </div>
                <div className="flex flex-col gap-2">
                  {pendingAcctPurchases.map(p => (
                    <div key={p.id} className="bg-white border border-amber-200/60 rounded-xl p-3 flex justify-between items-center gap-3">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="font-bold text-amber-800 text-xs">{p.vendor}</span>
                          <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded-full ${p.paymentType === '月結' ? 'bg-blue-100 text-blue-700' : 'bg-rose-100 text-rose-700'}`}>
                            {p.paymentType || '現結'}
                          </span>
                        </div>
                        <p className="text-[10px] text-amber-600 mt-0.5 line-clamp-1">{p.notes || ''}</p>
                        <span className="text-[10px] text-stone-400">{p.date}</span>
                      </div>
                      <div className="flex flex-col items-end gap-1">
                        <span className="font-bold text-amber-700 text-sm">${fmt(p.totalAmount)}</span>
                        <button
                          onClick={() => handleConfirmSchedulerPurchase(p)}
                          className="text-[10px] font-bold px-2.5 py-1 bg-amber-500 hover:bg-amber-600 text-white rounded-lg transition flex items-center gap-1"
                        >
                          <BadgeCheck className="w-3 h-3" /> 確認入帳
                        </button>
                      </div>
                    </div>
                  ))}
          ...
          <div className="space-y-3">
            {purchases.filter(p => !p.fromScheduler || p.accountingStatus === 'confirmed').slice(0, 20).map(p => (
                      ...
                      <div className="md:col-span-3">
                        ...
                        <div className="relative">
                          <input type="number" step="0.01" required value={line.purchaseQty !== undefined ? (line.purchaseQty || '') : (line.qty || '')} onChange={e => {
                            ...
                            const isMidUnit = currentUnit === mat?.midUnit;
                            const rate = isPurchaseUnit ? (mat?.purchaseUnitRate || 1) : (isMidUnit ? (mat?.midUnitRate || 1) : 1);
                            updateLine(line.id, { 
                              purchaseQty: pQty,
                          ...
                          {materialMap[line.materialId]?.purchaseUnit || materialMap[line.materialId]?.midUnit ? (
                             <select className="absolute right-0 top-0 bottom-0 bg-transparent text-coffee-500 font-bold text-xs px-2 outline-none cursor-pointer appearance-none text-center hover:bg-black/5 transition"
                               value={line.purchaseUnit || materialMap[line.materialId]?.purchaseUnit || materialMap[line.materialId]?.midUnit || materialMap[line.materialId]?.unit}
                               onChange={e => {
                                 const newUnit = e.target.value;
                                 const pQty = line.purchaseQty !== undefined ? line.purchaseQty : line.qty;
                                 const mat = materialMap[line.materialId];
                                 const isPurchaseUnit = newUnit === mat?.purchaseUnit;
                                 const isMidUnit = newUnit === mat?.midUnit;
                                 const rate = isPurchaseUnit ? (mat?.purchaseUnitRate || 1) : (isMidUnit ? (mat?.midUnitRate || 1) : 1);
                                 updateLine(line.id, {
                             ...
                             >
                               {materialMap[line.materialId]?.purchaseUnit && <option value={materialMap[line.materialId]?.purchaseUnit!}>{materialMap[line.materialId]?.purchaseUnit}</option>}
                               {materialMap[line.materialId]?.midUnit && <option value={materialMap[line.materialId]?.midUnit!}>{materialMap[line.materialId]?.midUnit}</option>}
                ...
                <form onSubmit={handleAddMaterial} className="space-y-6">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      ...
                      <div>
                        <label className="text-[10px] font-bold text-amber-500 block mb-1">最低庫存提醒水位 (最大單位) *</label>
                        <div className="relative">
