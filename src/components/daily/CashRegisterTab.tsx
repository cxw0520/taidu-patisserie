...
export default function CashRegisterTab({ dailyData, settings, updateDaily, metrics }: CashRegisterTabProps) {
  ...
  const handleCloseShift = () => {
    ...
    // expected = opening + cash sales - expenses
    const cashSales = dailyData.orders
      .filter(o => o.status === '現結')
      .reduce((sum, o) => sum + (o.actualAmt || 0), 0);
    
    // We should clarify metrics.cash vs manual cash register shift cash. 
    // For now, use the orders for simplicity.
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 h-[calc(100vh-250px)]">
      ...
      {/* Right: Cart (4 cols) */}
      <div className="lg:col-span-4 bg-white border border-coffee-100 rounded-3xl flex flex-col overflow-hidden shadow-lg">
